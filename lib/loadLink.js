...
export function identificacao(link) {
  const out = {};
  // codcli primeiro: há clientes com CPF gravado no campo de CNPJ, e mandar esse "CNPJ" faz a
  // API recusar a chamada inteira mesmo com um codcli válido junto.
  if (link.codcli) out.codcli = link.codcli;
  else if (link.cnpj) out.cnpj = link.cnpj;
